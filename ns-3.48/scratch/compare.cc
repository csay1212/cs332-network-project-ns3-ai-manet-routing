...
private:
  ...
  // Reproducibility parameters (optional CLI args)
  uint32_t m_seed;
  double m_simTime;
  int m_nNodes;
};
...
RoutingExperiment::CommandSetup (int argc, char **argv)
{
  ...
  cmd.AddValue ("seed",          "RNG seed for reproducibility",         m_seed);
...
RoutingExperiment::Run (int nSinks, double txp, std::string CSVfileName)
{
  ...
  int nWifis = m_nNodes; // now configurable via --nNodes

  double TotalTime = m_simTime; // now configurable via --simTime
